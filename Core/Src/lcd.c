...
//LCD的画笔颜色和背景色
u32 POINT_COLOR=0xFFFFFFFF;		//画笔颜色
u32 BACK_COLOR =0x002E8B57;  	//背景色
//管理LCD重要参数
//默认为竖屏
_lcd_dev lcddev;
...
void LCD_Draw_CircleColor(u16 x0,u16 y0,u8 r,u8 ex,u8 ey,u16 color)
{
	...
	while(a<=b)
	{
		LCD_Color_Fill(x0+a,y0-b,ex,ey,color);
		 osDelay(100);
		//LCD_DrawPoint(x0+a,y0-b);             //5
		LCD_Color_Fill(x0+b,y0-a,ex,ey,color);
		 osDelay(100);
...
{  							  
    u8 temp,t1,t;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数、问号表达式(size%8)?1:0)等于1或0
...
//重写字符显示，大字体的变量要定义为u16,特别是字节数组里面超过64*64的字体后需要将u8 temp,t1,t重定义为 u16不然不会显示；
void LCD_ShowChar1(u16 x,u16 y,u8 num,u8 size,u32 color,u8 mode)
{
    u16 temp,t1,t;
	u16 y0=y;
	u16 csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数、问号表达式(size%8)?1:0)等于1或0
	num=num-' ';//得到偏移后的值（ASCII字库是从空格开始取模，所以-' '就是对应字符的字库）
	for(t=0;t<csize;t++)
	{
		if(size==148)temp=asc2_14874[num][t];	//调用7236字体
		else if(size==16)temp=asc2_1608[num][t];	//调用1608字体
		else if(size==24)temp=asc2_2412[num][t];	//调用2412字体
		else if(size==32)temp=asc2_3216[num][t];	//调用3216字体
		else return;								//没有的字库
...
//显示自定义大字体，注意char*p所指的位置与PClcd取模位置对应，不与字符对应
